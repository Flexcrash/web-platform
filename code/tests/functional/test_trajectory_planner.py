...
def test_plot_and_check(xml_scenario_template):
    ...
    scenario_template = MixedTrafficScenarioTemplate(template_id, template_name, template_description, xml_scenario_template)

    duration = 1.0
    drivers = [user]

    mixed_traffic_scenario = MixedTrafficScenario(
        scenario_id, name, description, created_by, max_players, status, scenario_template, duration,
        drivers
    )
    ...
    initial_state = VehicleState(*initial_state_array)
