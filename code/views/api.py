...
from views.authentication import authentication_api, token_auth_engine
import jwt
...
api_layer.register_blueprint(training_scenarios_api)
...
def log_request_info():
    ...
    current_app.logger.debug("Request: {} at {} ".format(request.method, request.url))
...
def assertion_error_handler(error):
    # TODO Extract validation  message from AssertionError
    current_app.logger.debug("INVALID REQUEST Request: {}".format(error))
