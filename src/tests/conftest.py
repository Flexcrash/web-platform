...
def flexcrash_test_app(tmp_path):
    ...
    temp_database_sqlalchemy = f"sqlite:///{temp_database_file}"
...
MAX_INIT_SPEED_M_S = 36.0 # Circa 90 Km/h - 36.0 # Circa 130 Km/h
# Vehicle dimensions -
VEHICLE_LENGTH = 3.0
VEHICLE_WIDTH = 2.0
# This creates and issue in the scheduler because app_context() is not callable anymore?
SCHEDULER_API_ENABLED = False 
# SCHEDULER_EXECUTORS = dict()
# SCHEDULER_EXECUTORS["rendering"] = dict()
# SCHEDULER_EXECUTORS["rendering"]["type"] = "processpool"
# SCHEDULER_EXECUTORS["rendering"]["max_workers"] = 4
# SCHEDULER_EXECUTORS["driving"] = dict()
# SCHEDULER_EXECUTORS["driving"]["type"] = "processpool"
# SCHEDULER_EXECUTORS["driving"]["max_workers"] = 4
""".format(temp_database_file, temp_database_sqlalchemy, temp_scenario_image_folder, temp_scenario_image_folder, temp_scenario_image_folder)
...
def flexcrash_test_app_with_a_scenario_template_and_given_users(flexcrash_test_app,
                                                              ...
                                                              xml_scenario_template, mocker):
    def _method(users_id, scenario_template_id):

        # We mock the functions imported
        mocked_function = mocker.patch('views.scenario.deploy_av_in_background')
        mocked_function.side_effect = _nop
        mocked_function = mocker.patch('views.scenario.undeploy_av')
        mocked_function.side_effect = _nop
        ...
        mixed_traffic_scenario_template_dao.insert_and_get(
            MixedTrafficScenarioTemplate(template_id=scenario_template_id,
                                         name="xml",
                                         description="xml",
...
@pytest.fixture
def user_dao(flexcrash_test_app) -> UserDAO:
...
@pytest.fixture
def mixed_traffic_scenario_template_dao(flexcrash_test_app) -> MixedTrafficScenarioTemplateDAO:
    # TODO Eventually remove this. We still need appconfig here!
...
@pytest.fixture
def vehicle_state_dao(flexcrash_test_app, mixed_traffic_scenario_dao):
    return VehicleStateDAO(flexcrash_test_app.config, mixed_traffic_scenario_dao)