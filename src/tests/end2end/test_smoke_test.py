...
def test_end2end_api_scenario_evolution(xml_scenario_template_as_file, flexcrash_test_app, flexcrash_user_generator):
    ...
    with flexcrash_test_app.test_client() as test_client:
        ...
        scenario_data = generate_scenario_data(scenario_creator.user_id, 1, 0, 1, 5.0, 1, [])
        ...
        vehicle_state_schema = VehicleStateSchema(load_instance=True, many=True)
        # We need to transform the string into a JSON object
        vehicle_states = vehicle_state_schema.load(json.loads(response.data.decode("utf-8")), session=db.session)
        ...
        for vehicle_state in vehicle_states:
            if vehicle_state.timestamp == 0:
                ...
                assert vehicle_state.status == "PENDING", f"Vehicle at timestamp {vehicle_state.timestamp} has wrong state"
